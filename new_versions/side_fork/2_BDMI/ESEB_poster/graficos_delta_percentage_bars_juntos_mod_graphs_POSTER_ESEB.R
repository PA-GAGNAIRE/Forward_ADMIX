...
mutations = ggplot(data, aes(y = num, fill= result, x=result))+
  ...
  facet_wrap(~parent, ncol = 2, nrow = 1, strip.position = "top")+
  #coord_flip()+
  ...
  theme(panel.background = element_rect(fill = "white", colour = "black", size = 0.5),
        ...
        strip.text.x = element_text(size = 30, color = "black", face = "bold.italic"),
        axis.text.y = element_text(angle=0, colour = "black", size = 24),
        #axis.title.x = element_text(size = 16),
...
png(paste0("005_mutation_delta_effect_result_ANOVA_", dia, ".png"), width = 21, height = 21, units = "cm", res = 300)
mutations
dev.off()
