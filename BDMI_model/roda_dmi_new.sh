...
#dir=/home/pagagnaire/Documents/Flavia/dmi_fts
...
#max number of incompatibilities
dmi=2000
#number max loci that can evolve under load
load=20000
...
for i in "${run[@]}"; do
    ...
    python3.4 ./launch_sim_avdqnf.py nPop1=$(echo $p1) nPop2=$(echo $p2) nPop3=$(echo $p3) Lchro=$(echo $len) incomp=$(echo $dmi) outputFile="${i}" func_el=$(echo $load) mu=$(echo $mut) prop1=$(echo $prop1) prop2=$(echo $prop2) slimScript=dmi_avdqnf.slim > "${i}"_sim_log.txt
                                        #gen of admixture   #second samp. interval start        #end first samp interval  #end second samp interval   #samp step 1  #samp step 2    #log of time ancestry  
