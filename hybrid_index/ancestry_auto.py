...
#temporal samples
gen = ["10", "20", "30", "50", "70", "100", "200", "300", "400"]

   
for i in gen:
    filename = run + "_" + i + ".trees"
    ...
    with open(("ancestry_" + i + ".txt"), "wb") as f:
        for line in anc:
            np.savetxt(f, line, fmt="%.4f", delimiter="\n")
    
    
    mat = np.matrix(breaks)
    with open(("breaks_" + i + ".txt"), "wb") as f:
        for line in mat:
            np.savetxt(f, line, delimiter="\n", fmt="%e")

    
    time_end = time.time() - time_start
    print("Total time for ", i, "generations: ", time_end)
