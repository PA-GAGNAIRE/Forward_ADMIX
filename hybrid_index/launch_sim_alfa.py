...
for i in sys.argv:
	...
	if i[0] == 'slimScript':
		slimScript = i[1]
	if i[0] == 'nSampleAncestry':
		nSampleAncestry = int(i[1])

# Run slim
print('Run slim')
cmd = 'slim -d nPop1={0} -d nPop2={1} -d nPop3={2} -d Lchro={3} -d "Alfa={4}" -d "outputFile={5}" {6}'.format(nPop1, nPop2, nPop3, Lchro, Alfa, outputFile, slimScript)
os.system(cmd)
